...
void connect(TreeLinkNode *root) {
    ...
    if (root->next) {
         ...
         TreeLinkNode *left=nullptr, *right=nullptr;
         left = root->right ? root->right: root->left;

         for (TreeLinkNode *p = root->next; p!=nullptr; p=p->next) {
             if (p->left){
                 right = p->left;
                 break;
             }
...
void connect1(TreeLinkNode *root) {
    ...
    while(v.size()>0){
        ...
        if ( (root->left && root->left->next==nullptr) || (root->right && root->right->next==nullptr) ) {
